...
type CompanyConsultationsFormData = {
    email: string,
    topic: string,
    companyName: string,
    when: string,
...
export default function WorkshopFormPage({user}: { user: PublicUser }) {
    ...
    const {register, handleSubmit, errors} = useForm<CompanyConsultationsFormData>();

    const onSubmit = (data: CompanyConsultationsFormData) => {
        if (!buttonEnabled) {
            return
        }
    ...
    return <>
        <KtAForm
            ...
            <TextEdit
                question={t.user.trainer.consultationForm.companyName}
